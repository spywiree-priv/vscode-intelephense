...
export async function activate(context: ExtensionContext) {
  ...
  let indexWorkspaceCmdDisposable = commands.registerCommand(
    INDEX_WORKSPACE_CMD_NAME,
    indexWorkspace
  );
  let cancelIndexingCmdDisposable = commands.registerCommand(
    CANCEL_INDEXING_CMD_NAME,
    cancelIndexing
  );
  let enterKeyCmdDisposable = commands.registerCommand(ENTER_KEY_CMD_NAME, () =>
    enterLicenceKey(context)
  );
...
function createClient(
  ...
  middleware: IntelephenseMiddleware,
  clearCache: boolean
) {
  let serverModule: string;
  if (process.env.mode === "debug") {
    serverModule = context.asAbsolutePath(
      path.join("node_modules", "intelephense", "out", "server.js")
    );
  } else {
    serverModule = context.asAbsolutePath(
      path.join("node_modules", "intelephense", "lib", "intelephense.js")
    );
  }
...
function registerNotificationListeners() {
  ...
  languageClient.onNotification(INDEXING_STARTED_NOTIFICATION.method, () => {
    window.setStatusBarMessage(
      "$(sync~spin) intelephense " + VERSION.toString() + " indexing ...",
